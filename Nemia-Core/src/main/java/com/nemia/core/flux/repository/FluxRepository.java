...
@Query("SELECT COALESCE(SUM(f.montant), 0) FROM Flux f WHERE f.statutJustificatif IN ('A_FOURNIR', 'INCOMPLET') AND f.date >= :debut AND f.date <= :fin")
...
@Query("SELECT COALESCE(SUM(f.montant), 0) FROM Flux f WHERE f.qualificationPressentie = 'A_ARBITRER' AND f.date >= :debut AND f.date <= :fin")
...
@Query("SELECT f.categorie, COALESCE(SUM(f.montant), 0) FROM Flux f WHERE f.type = 'DEPENSE' AND f.date >= :debut AND f.date <= :fin GROUP BY f.categorie ORDER BY SUM(f.montant) DESC")
List<Object[]> sumDepensesParCategorie(@Param("debut") LocalDate debut, @Param("fin") LocalDate fin);

// ── Récurrence dépenses ──

@Query("SELECT COALESCE(SUM(f.montant), 0) FROM Flux f WHERE f.type = 'DEPENSE' AND f.occurrence = 'RECURRENT' AND f.date >= :debut AND f.date <= :fin")
BigDecimal sumDepensesRecurrentes(@Param("debut") LocalDate debut, @Param("fin") LocalDate fin);

@Query("SELECT COALESCE(SUM(f.montant), 0) FROM Flux f WHERE f.type = 'DEPENSE' AND f.occurrence = 'PONCTUEL' AND f.date >= :debut AND f.date <= :fin")
...
List<Flux> findDernieresOperations(@Param("debut") LocalDate debut, @Param("fin") LocalDate fin, org.springframework.data.domain.Pageable pageable);
}
