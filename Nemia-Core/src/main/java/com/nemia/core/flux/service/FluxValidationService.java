...
public class FluxValidationService {
    ...
    public void validateBlocking(FluxType type, FluxCategory categorie) {
        ...
        if (type == FluxType.RECETTE && CATEGORIES_INTERDITES_RECETTE.contains(categorie)) {
            throw new IllegalArgumentException(
                    "Une recette ne peut pas être classée dans une catégorie de dépense ou de mouvement financier"
            );
        }

        if (type == FluxType.DEPENSE && CATEGORIES_INTERDITES_DEPENSE.contains(categorie)) {
            throw new IllegalArgumentException(
                    "Une dépense ne peut pas être classée dans une catégorie de recette ou de mouvement financier"
            );
        }

        if (type == FluxType.MOUVEMENT_FINANCIER && CATEGORIES_INTERDITES_MOUVEMENT_FINANCIER.contains(categorie)) {
            throw new IllegalArgumentException(
                    "Un mouvement financier ne peut pas être classé dans une catégorie de charge ou de recette"
            );
        }
    }
    ...
    public List<String> computeWarnings(
        ...
        if (qualification == QualificationPressentie.IMMOBILISATION
                && CATEGORIES_CHARGE_COURANTE_MANIFESTE.contains(categorie)) {
        ...
        if (qualification == QualificationPressentie.CHARGE_COURANTE
                && CATEGORIES_TRAVAUX_MOBILIER.contains(categorie)) {
        ...
        if (statutJustificatif == StatutJustificatif.NON_REQUIS
                ...
                && CATEGORIES_CHARGE_COURANTE.contains(categorie)) {
        ...
        if (occurrence == Occurrence.PONCTUEL
                && CATEGORIES_STRUCTURELLEMENT_RECURRENTES.contains(categorie)) {
