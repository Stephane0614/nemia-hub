...
public class FluxValidationService {
  ...
  private static final Set<FluxCategory> CATEGORIES_RECETTE = Set.of(FluxCategory.LOYER);
  ...
  private void validateBlocking(FluxType type, FluxCategory categorie) {
    ...
    if (type == FluxType.RECETTE && CATEGORIES_DEPENSE.contains(categorie)) {
      throw new IllegalArgumentException("Incohérence bloquante : une RECETTE ne peut pas avoir la catégorie " + categorie.getLabel());
    }

    if (type == FluxType.DEPENSE && CATEGORIES_RECETTE.contains(categorie)) {
      throw new IllegalArgumentException("Incohérence bloquante : une DEPENSE ne peut pas avoir la catégorie " + categorie.getLabel());
    }
  ...
  private List<String> collectWarnings(
    ...
    if (qualification == QualificationPressentie.IMMOBILISATION && categorie != null && CATEGORIES_COURANTES.contains(categorie)) {
    ...
    if (
      statutJustificatif == StatutJustificatif.NON_REQUIS &&
    ...
    if (occurrence == Occurrence.PONCTUEL && categorie != null && CATEGORIES_RECURRENTES.contains(categorie)) {
