...
void setup() {
    ...
    void shouldCreateFlux() throws Exception {
        FluxResponse response = buildFluxResponse(
                1L, LocalDate.of(2026, 4, 15), FluxType.RECETTE,
                "Loyer avril", new BigDecimal("850.00"),
                FluxCategory.LOYER, PaymentMode.VIREMENT, "Payé le 5");
        ...
        mockMvc.perform(post("/api/flux").contentType(MediaType.APPLICATION_JSON).content(requestBody))
                .andDo(print())
    ...
    void shouldReturnAllFlux() throws Exception {
        ...
        FluxResponse second = buildFluxResponse(
                2L, LocalDate.of(2026, 4, 16), FluxType.DEPENSE,
                "Internet", new BigDecimal("29.99"),
                FluxCategory.INTERNET, PaymentMode.PRELEVEMENT, null);
    ...
    void shouldReturnFluxById() throws Exception {
        Long id = 1L;
        FluxResponse response = buildFluxResponse(
                id, LocalDate.of(2026, 4, 15), FluxType.RECETTE,
                "Loyer avril", new BigDecimal("850.00"),
                FluxCategory.LOYER, PaymentMode.VIREMENT, "Payé le 5");

        when(fluxService.findById(id)).thenReturn(response);

        mockMvc.perform(get("/api/flux/{id}", id))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(1))
                .andExpect(jsonPath("$.libelle").value("Loyer avril"));
    ...
    void shouldUpdateFlux() throws Exception {
        Long id = 1L;
        FluxResponse response = buildFluxResponse(
                id, LocalDate.of(2026, 4, 15), FluxType.DEPENSE,
                "Internet avril", new BigDecimal("29.99"),
                FluxCategory.INTERNET, PaymentMode.PRELEVEMENT, null);
        ...
        mockMvc.perform(put("/api/flux/{id}", id).contentType(MediaType.APPLICATION_JSON).content(requestBody))
                .andDo(print())
                .andExpect(status().isOk())
    ...
    void shouldDeleteFlux() throws Exception {
        ...
        mockMvc.perform(delete("/api/flux/{id}", id))
                .andDo(print())
    ...
    void shouldReturnWarningsWhenOccurrencePonctuelOnRecurrentCategory() throws Exception {
        FluxResponse response = buildFluxResponse(
                1L, LocalDate.of(2026, 4, 15), FluxType.DEPENSE,
                "Charges copro", new BigDecimal("200.00"),
                FluxCategory.CHARGES_COPRO, PaymentMode.VIREMENT, null);
    ...
    void shouldRejectRecetteWithDepenseCategory() throws Exception {
        when(fluxService.create(any())).thenThrow(
                new IllegalArgumentException("Incohérence bloquante"));
    ...
    private FluxResponse buildFluxResponse(Long id, LocalDate date, FluxType type,
                                            String libelle, BigDecimal montant,
                                            FluxCategory categorie, PaymentMode modePaiement,
                                            String commentaire) {
