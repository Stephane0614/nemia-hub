...
class FluxFilterControllerTest {
  ...
  void findAll_sansFiltre_retourneTousLesFlux() throws Exception {
    ...
    )).thenReturn(pageOf(buildFluxResponse(1L), buildFluxResponse(2L)));
  ...
  void findAll_avecBienId_retourneFluxFiltres() throws Exception {
    when(fluxService.findAllWithFilters(
      eq(1L), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq(0), eq(20)
  ...
  void findAll_avecBienId_sansResultat_retournePageVide() throws Exception {
    when(fluxService.findAllWithFilters(
      eq(99L), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq(0), eq(20)
    )).thenReturn(emptyPage());

    mockMvc.perform(get("/api/flux").param("bienId", "99"))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.contenu").isArray())
  ...
  void findAll_avecQualificationPressentie_retourneFluxFiltres() throws Exception {
    when(fluxService.findAllWithFilters(
      isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq("A_ARBITRER"), isNull(), isNull(), eq(0), eq(20)
    )).thenReturn(pageOf(buildFluxResponse(1L)));
  ...
  void findAll_avecStatutTraitement_retourneFluxFiltres() throws Exception {
    when(fluxService.findAllWithFilters(
      isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq("A_REVOIR"), isNull(), eq(0), eq(20)
    )).thenReturn(pageOf(buildFluxResponse(1L)));
  ...
  void findAll_avecStatutJustificatifMultiple_retourneFluxFiltres() throws Exception {
    when(fluxService.findAllWithFilters(
      isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq("A_FOURNIR,INCOMPLET"), eq(0), eq(20)
    )).thenReturn(pageOf(buildFluxResponse(1L)));
  ...
  void findAll_avecExerciceId_retourneFluxFiltres() throws Exception {
    when(fluxService.findAllWithFilters(
      isNull(), eq(2L), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq(0), eq(20)
    )).thenReturn(pageOf(buildFluxResponse(1L)));

    mockMvc.perform(get("/api/flux").param("exerciceId", "2"))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.contenu.length()").value(1));
  ...
  void findAll_avecExerciceId_sansResultat_retournePageVide() throws Exception {
    when(fluxService.findAllWithFilters(
      isNull(), eq(99L), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq(0), eq(20)
    )).thenReturn(emptyPage());

    mockMvc.perform(get("/api/flux").param("exerciceId", "99"))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.totalElements").value(0));
  }
  ...
  void findAll_avecTypeFlux_retourneFluxFiltres() throws Exception {
    when(fluxService.findAllWithFilters(
      isNull(), isNull(), eq("RECETTE"), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq(0), eq(20)
    )).thenReturn(pageOf(buildFluxResponse(1L)));

    mockMvc.perform(get("/api/flux").param("typeFlux", "RECETTE"))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.contenu.length()").value(1));
  }
  ...
  void findAll_avecTypeFlux_sansResultat_retournePageVide() throws Exception {
    when(fluxService.findAllWithFilters(
      isNull(), isNull(), eq("MOUVEMENT_FINANCIER"), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq(0), eq(20)
    )).thenReturn(emptyPage());

    mockMvc.perform(get("/api/flux").param("typeFlux", "MOUVEMENT_FINANCIER"))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.totalElements").value(0));
  }
  ...
  void findAll_avecCategorie_retourneFluxFiltres() throws Exception {
    when(fluxService.findAllWithFilters(
      isNull(), isNull(), isNull(), eq("LOYER"), isNull(), isNull(), isNull(), isNull(), isNull(), eq(0), eq(20)
    )).thenReturn(pageOf(buildFluxResponse(1L)));

    mockMvc.perform(get("/api/flux").param("categorie", "LOYER"))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.contenu.length()").value(1));
  }
  ...
  void findAll_avecCategorie_sansResultat_retournePageVide() throws Exception {
    when(fluxService.findAllWithFilters(
      isNull(), isNull(), isNull(), eq("TRAVAUX"), isNull(), isNull(), isNull(), isNull(), isNull(), eq(0), eq(20)
    )).thenReturn(emptyPage());

    mockMvc.perform(get("/api/flux").param("categorie", "TRAVAUX"))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.totalElements").value(0));
  }
  ...
  void findAll_avecPeriode_retourneFluxFiltres() throws Exception {
    when(fluxService.findAllWithFilters(
      isNull(), isNull(), isNull(), isNull(), eq("2026-04-01"), eq("2026-04-30"), isNull(), isNull(), isNull(), eq(0), eq(20)
    )).thenReturn(pageOf(buildFluxResponse(1L)));
  ...
  void findAll_avecPeriode_sansResultat_retournePageVide() throws Exception {
    when(fluxService.findAllWithFilters(
      isNull(), isNull(), isNull(), isNull(), eq("2020-01-01"), eq("2020-01-31"), isNull(), isNull(), isNull(), eq(0), eq(20)
    )).thenReturn(emptyPage());

    mockMvc.perform(get("/api/flux")
        .param("dateDebut", "2020-01-01")
        .param("dateFin", "2020-01-31"))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.totalElements").value(0));
  }
  ...
  void findAll_premierePage_retourneMetadonneesPagination() throws Exception {
    ...
    when(fluxService.findAllWithFilters(
      isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq(0), eq(2)
    )).thenReturn(page);

    mockMvc.perform(get("/api/flux").param("page", "0").param("size", "2"))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.contenu.length()").value(2))
  ...
  void findAll_pagesuivante_retourneMetadonneesPagination() throws Exception {
    ...
    when(fluxService.findAllWithFilters(
      isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq(1), eq(2)
    )).thenReturn(page);

    mockMvc.perform(get("/api/flux").param("page", "1").param("size", "2"))
      .andExpect(status().isOk())
  ...
  void findAll_taillePersonnalisee_retourneMetadonneesPagination() throws Exception {
    List<FluxResponse> contenu = List.of(
      buildFluxResponse(1L), buildFluxResponse(2L), buildFluxResponse(3L),
      buildFluxResponse(4L), buildFluxResponse(5L)
    );
    FluxPageResponse page = new FluxPageResponse(contenu, 0, 50, 5L, 1);

    when(fluxService.findAllWithFilters(
      isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), isNull(), eq(0), eq(50)
    )).thenReturn(page);

    mockMvc.perform(get("/api/flux").param("size", "50"))
      .andExpect(status().isOk())
  ...
  void findAll_avecFiltresCombinesEtPagination_retourneResultatFiltrePagine() throws Exception {
    FluxPageResponse page = new FluxPageResponse(
      List.of(buildFluxResponse(1L)), 0, 5, 1L, 1
    );

    when(fluxService.findAllWithFilters(
      eq(1L), isNull(), eq("DEPENSE"), isNull(), isNull(), isNull(), isNull(), isNull(), eq("A_FOURNIR,INCOMPLET"), eq(0), eq(5)
    )).thenReturn(page);
