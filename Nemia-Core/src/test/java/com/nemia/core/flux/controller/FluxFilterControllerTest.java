...
class FluxFilterControllerTest {
  ...
  void findAll_sansFiltre_retourneTousLesFlux() throws Exception {
    when(fluxService.findAllWithFilters(null, null, null, null)).thenReturn(List.of(buildFluxResponse(1L), buildFluxResponse(2L)));
  ...
  void findAll_avecBienId_retourneFluxFiltres() throws Exception {
    when(fluxService.findAllWithFilters(eq(1L), isNull(), isNull(), isNull())).thenReturn(List.of(buildFluxResponse(1L)));

    mockMvc.perform(get("/api/flux").param("bienId", "1")).andExpect(status().isOk()).andExpect(jsonPath("$.length()").value(1));
  ...
  void findAll_avecQualificationPressentie_retourneFluxFiltres() throws Exception {
    when(fluxService.findAllWithFilters(isNull(), eq("A_ARBITRER"), isNull(), isNull())).thenReturn(List.of(buildFluxResponse(1L)));
  ...
  void findAll_avecStatutTraitement_retourneFluxFiltres() throws Exception {
    when(fluxService.findAllWithFilters(isNull(), isNull(), eq("A_REVOIR"), isNull())).thenReturn(List.of(buildFluxResponse(1L)));
  ...
  void findAll_avecStatutJustificatifMultiple_retourneFluxFiltres() throws Exception {
    when(fluxService.findAllWithFilters(isNull(), isNull(), isNull(), eq("A_FOURNIR,INCOMPLET"))).thenReturn(
      List.of(buildFluxResponse(1L))
    );
  ...
  void findAll_avecFiltresCombines_retourneFluxFiltres() throws Exception {
    when(fluxService.findAllWithFilters(eq(1L), isNull(), isNull(), eq("A_FOURNIR,INCOMPLET"))).thenReturn(List.of(buildFluxResponse(1L)));

    mockMvc
      .perform(get("/api/flux").param("bienId", "1").param("statutJustificatif", "A_FOURNIR,INCOMPLET"))
      .andExpect(status().isOk())
      .andExpect(jsonPath("$.length()").value(1));
  ...
  void findAll_avecFiltreVide_retourneTousLesFlux() throws Exception {
    ...
    mockMvc.perform(get("/api/flux")).andExpect(status().isOk()).andExpect(jsonPath("$.length()").value(2));
  }
}
